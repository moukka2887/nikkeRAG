...
```
nikkerag/
├── docs/                           # 📚 文檔目錄（詳見 docs/README.md）
│   ├── methodology/                # RAG 生成方法論
│   ├── timeline/                   # 故事時間線分析
│   ├── validation/                 # 字幕校準與質量驗證
│   ├── references/                 # 參考文獻（GraphRAG 論文等）
│   ├── calibration_reports/        # 字幕校準報告
│   ├── characters/                 # 角色檔案（13個）
│   ├── videos/                     # RAG 文檔（109個）
│   │   ├── NIKKE 第XX章 - RAG文档.md      # AI 查詢優化版
│   │   └── NIKKE 第XX章 - 原文記錄.md     # 完整原始對話
│   └── worldbuilding/              # 世界觀設定
│
├── scripts/                        # 🔧 處理腳本
│   ├── test_youtube_access.py      # YouTube 訪問測試
│   ├── whisper_asr_batch.py        # Whisper 語音識別
│   ├── batch_extract_all.py        # 批量提取字幕
│   ├── batch_generate_rag.py       # 批量生成 RAG
│   └── convert_vtt_to_json.py      # VTT 轉 JSON
│
├── templates/                      # 📄 文檔模板
│   ├── rag_document_template.md    # RAG 文檔模板
│   ├── change_proposal_template.md # 變更提案模板
│   └── state_file_template.json    # 狀態文件模板
│
├── temp/                           # 🗂️ 臨時文件
│   ├── *_calibrated.json           # 校準後的字幕
│   ├── *_transcript.json           # 原始字幕
│   └── *.vtt                       # VTT 字幕文件
│
├── archive/                        # 📦 歸檔文件
├── unused/                         # 🚫 廢棄文件
├── CLAUDE.md                       # Claude Code 項目指南
├── NIKKE_TERMINOLOGY.md            # NIKKE 術語標準（50+條目）
└── README.md                       # 本文件
```
